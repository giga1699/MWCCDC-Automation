...
authentikMasterOrangeGroup="20fc91be-8636-4afa-9774-a49b999fd939"
...
if os.getenv('authentikMasterOrangeGroup'):
    teamPassCSV=os.getenv('authentikMasterOrangeGroup')
if os.getenv('authentikMasterRedGroup'):
...
## ORANGE TEAM
# Get a list of orange team usernames from authentik
payload = json.dumps({
    "include_children": True,
})
response = requests.request("GET", f'https://{authentikDomain}/api/v3/core/groups/{authentikMasterOrangeGroup}/', headers=authentikHeaders, params=payload)
if debug:
    print(response.status_code, response.text)

if response.status_code != 200:
    zulip.sendChannelMessage(zulipOperationsChannel, zulipOperationsTopic, f'@*Black Team* **ERROR**: Couldn\'t get a list of orange team users using group pk {authentikMasterOrangeGroup}')
    exit(-1)
...
for userObj in response.json()['users_obj']:
    ...
    if zulipUserID:
        try:
            zulipAdmin.addUserToGroup(zulipUserID, defaultZulipGroups['Orange Team'])
        except Exception as ex:
            if not re.search('User \d+ is already a member of this group', str(ex)):
                zulip.sendChannelMessage(zulipOperationsChannel, zulipOperationsTopic, f'**WARN**: Couldn\'t add Zulip user with email {userObj["email"]} to "Orange Team" group.\r\nException:\r\n```\r\n{ex}\r\n```')
    
    try:
        for channel in zulipChannels:
            if (re.match(zulipOrangeChannelRegex, channel['name']) or re.match("orange-team-internal", channel['name'])) and zulipUserID:
                zulipAdmin.subscribeUserToChannel(zulipUserID, channel['name'])
    except Exception as ex:
        zulip.sendChannelMessage(zulipOperationsChannel, zulipOperationsTopic, f'**WARN**: Error adding zulip ID {zulipUserID} to orange team channel(s).\r\nException:```\r\n{ex}\r\n```')
...
for userObj in response.json()['users_obj']:
    ...
    try:
        for channel in zulipChannels:
            if (re.match("red-team", channel['name']) or re.match("red-memes", channel['name']) or re.match(zulipRedChannelRegex, channel['name'])) and zulipUserID:
                zulipAdmin.subscribeUserToChannel(zulipUserID, channel['name'])
    except Exception as ex:
